...
class Embeddings(nn.Module):
    ...
    def forward(self, 
                ...
                input_embeds: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Parameters:
            input_ids (torch.Tensor):
...
class DistilBertForMaskedLM(DistilBertPreTrainedModel):
    ...
    def __init__(self, config: PretrainedConfig):
        ...
        self.cls = nn.Sequential(
            ...
            self.vocab_layer_norm,
            self.vocab_projector
        )

        # condenser add:
        self.decoder_heads = nn.ModuleList(
            [TransformerBlock(self.config) for _ in range(2)])
    ...
    def forward_decoder_heads(
        ...
        # print('position_ids', dec_position_ids, 'dec_token_type_ids', dec_token_type_ids, 'dec_past_key_values_length', dec_past_key_values_length)
        if dec_input_ids is not None:
            dec_embeddings = self.distilbert.embeddings(
                ...
                input_embeds=dec_inputs_embeds,
