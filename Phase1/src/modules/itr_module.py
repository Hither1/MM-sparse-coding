...
from .distilbert_model import DistilBertForMaskedLM
from .clip_vision_model import CLIPVisionModel
...
class ITRTransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        if torch.distributed.is_initialized():
            if torch.distributed.get_rank() == 0:
                ...
                DistilBertForMaskedLM.from_pretrained(config['tokenizer'])
            torch.distributed.barrier()
        
        self.image_transformer = CLIPVisionModel.from_pretrained("openai/clip-vit-base-patch32")
        self.text_transformer = DistilBertForMaskedLM.from_pretrained("distilbert-base-uncased")
    ...
    def encode_text(
        ...
        if self.training_mode == "bottle":
            ...
            pooled_enc_logits = torch.max(mlm_logits, dim=1)[0] # [bsz, vocab_size]
            pooled_enc_probs = torch.softmax(pooled_enc_logits, dim=-1) # [bsz, vocab_size]
            word_embeddings_matrix = self.text_transformer.distilbert.get_input_embeddings().weight.data.detach() # [vocab_size, 768]
            bottleneck_repre = torch.matmul(pooled_enc_probs, word_embeddings_matrix) # [bsz, 768]
        elif self.training_mode == "both":
            pooled_enc_logits = torch.max(mlm_logits, dim=1)[0] # [bsz, vocab_size]
            pooled_enc_probs = torch.softmax(pooled_enc_logits, dim=-1) # [bsz, vocab_size]
    ...
    def encoder_infer(
        ...
        # self.enc_img_time += time.time() - start_time
        # self.n += 1
        # print(f'Avg encode image time: {self.enc_img_time / self.n}')

        # start_time = time.time()
        image_bottleneck_repre = self.encode_image(
            image_features, image_masks=None,
            word_embeddings_matrix=self.text_transformer.distilbert.get_input_embeddings().weight.data.detach(),
        )
        # self.enc_img_time += time.time() - start_time
        # print(f'Avg encode image time: {self.enc_img_time / self.n}')
