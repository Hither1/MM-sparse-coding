...
class ITRTransformerSS(pl.LightningModule):
    def __init__(self, valid_token_id, config):
        super().__init__()
        ...
        self.channel = 3
        self.v_size = config["vocab_size"]
        self.cell_num = config["vocab_size"]*self.channel
    ...
    def encode_image(
        ...
        if self.training_mode == "bottle":
            ...
            # bottleneck_repre = pooled_enc_probs.reshape(bs, self.v_size, self.channel)
            # bottleneck_repre[:, self.valid_token_id] = 0
            # bottleneck_repre = bottleneck_repre.reshape(bs, self.v_size * self.channel)
    ...
    def _generate_index_and_value(self, reps, start, end, queue_index, queue_value):
        ...
        for b in range(start, end):
            ...
            queue_index[b, 0, :len(ids)] = torch.full((len(ids),), b).to(reps.device)
