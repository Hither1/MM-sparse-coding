...
def CL4MoCo(
    ...
    i, v = masked_index_and_value(pl_module.image_queue_index.clone().detach(),
                                  pl_module.image_queue_value.clone().detach())
    ...
    i, v = masked_index_and_value(pl_module.text_queue_index.clone().detach(),
...
def compute_contrastive(pl_module, ret):
    ...
    lambda_I = 2e-2
    lambda_T = 5e-2
    Overuse_loss = lambda_I * overuse_penalty(text_reps, pl_module.channel) + lambda_T * overuse_penalty(image_reps,
                                                                                                         pl_module.channel)
    # CH_FLOP_loss = 2 * (CHANNEL_FLOAP(text_reps, pl_module.channel)+CHANNEL_FLOAP(image_reps, pl_module.channel))
    ...
    new_ret = {
        "contrastive_loss": contrastive_loss + Overuse_loss,
    }
    ...
    # loss_ch = getattr(pl_module, f"{phase}_contrastive_loss")(CH_FLOP_loss)
    ...
    # pl_module.log(f"phase_2/{phase}/loss_ch", loss_ch)
