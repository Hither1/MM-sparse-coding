...
def CL4MoCo(
    ...
    # positive logits: N
    channel = pl_module.channel
    v_szie = pl_module.v_size
    bs = text_reps.shape[0]
    ...
    # crow, col, v = masked_csr_inputs(pl_module.image_queue_crow.clone().detach(), pl_module.image_queue_col.clone().detach(), pl_module.image_queue_value.clone().detach())
    # image_queue_tensor_s = torch.sparse_csr_tensor(crow, col, v, (queue_size, cell_num), dtype=torch.float16).to(text_reps.device)
    # print('image', image_queue_tensor_s)

    # crow2, col2, v2 = masked_csr_inputs(pl_module.text_queue_crow.clone().detach(),
    #                                  pl_module.text_queue_col.clone().detach(),
    #                                  pl_module.text_queue_value.clone().detach())
    # text_queue_tensor_s = torch.sparse_csr_tensor(crow2, col2, v2, (queue_size, cell_num), dtype=torch.float16).to(text_reps.device)
    # print('text', text_queue_tensor_s)
...
def L1_loss(batch_rep, channel, v_size):
...
def overuse_penalty(batch_rep, channel):
    ...
    N = batch_rep.shape[0]
    # nom = N * vocab_size * torch.sum(torch.mean(torch.abs(batch_rep), dim=0) ** 3)
    # denom = torch.sum(torch.sum(torch.abs(batch_rep), dim=0))
...
def compute_contrastive(pl_module, ret):
    ...
    # FLOAP_loss = 0.05 * (FLOAP(text_reps, pl_module.channel) + FLOAP(image_reps, pl_module.channel))

    beta = 0.001
    l1_loss_v = beta * L1_loss(text_reps, pl_module.channel, pl_module.v_size) + beta * L1_loss(image_reps,
                                                                                                      pl_module.channel,
                                                                                                      pl_module.v_size)
    ...
    new_ret = {
        "contrastive_loss": l1_loss_v + contrastive_loss,
    }
    ...
    loss_floap = getattr(pl_module, f"{phase}_contrastive_loss")(l1_loss_v)
    pl_module.log(f"phase_2/{phase}/loss_cts", loss)
    pl_module.log(f"phase_2/{phase}/loss_l1", loss_floap)
