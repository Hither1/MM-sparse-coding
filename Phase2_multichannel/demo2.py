...
def main(_config):
    ...
    for idx in range(1000):
        ...
        reps = img_reps
        vocab_size = reps.shape[-1] // 3
        bs, seq_len = reps.shape[:2]
        channel = 3
        mlm_logits_sum = torch.sum(reps.view(bs, seq_len, vocab_size, channel), dim=-1)
        ...
        print('img', img_dict)
        mlm_logits_ch0 = reps.view(bs, seq_len, vocab_size, channel)[:,:,:,0]
        ...
        mlm_logits_ch1 = reps.view(bs, seq_len, vocab_size, channel)[:, :, :, 1]
        ...
        mlm_logits_ch2 = reps.view(bs, seq_len, vocab_size, channel)[:, :, :, 2]
