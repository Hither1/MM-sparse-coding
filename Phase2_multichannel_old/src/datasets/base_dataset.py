...
class CsvDataset(torch.utils.data.Dataset):
    ...
    def get_image(self, idx):
        ...
        image_features_small = self.transforms_small[0](Image.open(f"{self.data_dir[:8]}/{file_name}")).unsqueeze(0)
        # image_features_small = self.transforms_small[0](Image.open(f"{self.data_dir[:7]}/{str(self.images[idx])[3:]}")).unsqueeze(0)
