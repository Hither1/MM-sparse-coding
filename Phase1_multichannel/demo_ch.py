...
def main(_config):
    ...
    score_1_list, score_2_list, score_3_list, score_4_list = [], [], [], []
    activated_by_3_channels = []
    ...
    for idx in range(3):
        ...
        mlm_logits_mul = img_reps
        channel = 3
        vocab_size = mlm_logits_mul.shape[-1] // channel
        bs = mlm_logits_mul.shape[0]
        mlm_logits_mul = mlm_logits_mul.view(bs, vocab_size, channel)
        mlm_logits_sum = torch.sum(mlm_logits_mul, dim=-1)
        mlm_logits_sum[:, invalid_token_id] = 0


        # img_dict = transform_sparse_vector_topk_torch(mlm_logits_sum[0], vocabulary_list, k=10)
        # print('img', img_dict)
        mlm_logits_ch0 = mlm_logits_mul[:, :, 0]
        mlm_logits_ch1 = mlm_logits_mul[:, :, 1]
        mlm_logits_ch2 = mlm_logits_mul[:, :, 2]
        ...
        for idx, ch in enumerate(index_avg):
            # print(mlm_logits_ch0[0, ch].item(), mlm_logits_ch1[0, ch].item(), mlm_logits_ch2[0, ch].item(), mlm_logits_ch3[0, ch].item())
            if ch in index_ch0:
                counter[idx] += 1
            if ch in index_ch1:
                counter[idx] += 1
            if ch in index_ch2:
                counter[idx] += 1
            if counter[idx] == 2:
                key = vocabulary_list[ch]
                polysemous_words.append(key)
                if key not in activated_by_3_channels:
                    activated_by_3_channels.append(key)
        if len(polysemous_words) > 0:
            print(text, 'tokens activated by 3 channels:', polysemous_words)
