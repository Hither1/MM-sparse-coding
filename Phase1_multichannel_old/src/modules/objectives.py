...
def CHANNEL_FLOAP(batch_rep, channel):
    batch_rep = batch_rep.reshape((batch_rep.shape[0], channel, -1))
    batch_rep = batch_rep.transpose(1, 2)

    return torch.sum(torch.mean(torch.abs(batch_rep) ** 2, dim=(0, 1)) )

def meaningless_penalty(batch_rep):
    N, V = batch_rep.shape[0], batch_rep.shape[1]
    nom = N * V * torch.sum(torch.mean(torch.abs(batch_rep), dim=0) ** 3)
...
def compute_contrastive(pl_module, ret):
    ...
    CH_FLOAP_loss = 0.002 * (CHANNEL_FLOAP(text_reps, pl_module.channel) + CHANNEL_FLOAP(image_reps, pl_module.channel))

    lambda_I = 5 * 10**-4
    lambda_T = 1 * 10**-3
    overuse_loss = lambda_T * meaningless_penalty(text_reps) + lambda_I * meaningless_penalty(image_reps)
    ...
    new_ret = {
        "contrastive_loss": contrastive_loss + FLOAP_loss + CH_FLOAP_loss + overuse_loss,
    }

    phase = "train" if pl_module.training else "val"
    loss = getattr(pl_module, f"{phase}_contrastive_loss")(new_ret["contrastive_loss"])
    pl_module.log(f"contrastive/{phase}/loss", loss)
    pl_module.log(f"contrastive/{phase}/FLOP_loss", FLOAP_loss)
    pl_module.log(f"contrastive/{phase}/Channel_FLOP_loss", CH_FLOAP_loss)
    pl_module.log(f"contrastive/{phase}/overuse_loss", overuse_loss)
